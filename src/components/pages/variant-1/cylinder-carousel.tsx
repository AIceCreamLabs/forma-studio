    ...
    images.forEach((src, i) => {
      ...
      img.onload = () => {
        ...
        const prog = new Program(gl as any, {
          ...
          uniforms: { tMap: { value: tex }, uDarkness: { value: 0.3 } },
          cullFace: null,
        });
        ...
        const animate = () => {
          ...
          const lerpRate = speedRef.current * 0.06 + 0.004; // min 0.4% per frame even when frozen
    ...
    <>
      ...
      <div className="fixed inset-0 pointer-events-none z-10 text-white">
        {perspectives.map((p, i) => (
          ...
          >
            <h2 className="text-7xl font-[300] max-md:text-3xl leading-[0.88] tracking-[-0.04em]">{p.title}</h2>
            {p.description && (
              <p className="text-2xl font-[300] max-md:text-base opacity-40 mt-3 tracking-[0.01em]">{p.description}</p>
            )}
          </div>
        ))}
      </div>
      ...
      {!hintHidden && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-20 pointer-events-none flex flex-col items-center gap-2">
          <svg width="18" height="26" viewBox="0 0 18 26" fill="none" className="opacity-30">
            <path d="M9 2L9 20M9 20L4 15M9 20L14 15" stroke="white" strokeWidth="0.8"/>
          </svg>
          <span className="text-[9px] tracking-[0.28em] uppercase text-white/30 font-mono">Scroll</span>
        </div>
      )}
      ...
      {hintHidden && (
        <div className="fixed bottom-8 right-8 z-20 pointer-events-none">
          <p className="text-[8px] tracking-[0.26em] uppercase text-white/20 font-mono">
            Click to discover
          </p>
        </div>
      )}
      ...
      {count > 0 && (
        <button
          ...
          className="fixed top-6 right-6 z-30 w-9 h-9 bg-white text-black text-[9px] font-mono tracking-wider flex items-center justify-center pointer-events-auto"
        >
          {count}
        </button>
      )}
